...
SETTINGS_FILE = "SublimePrettier.sublime-settings"
...
class PrettierCommand(sublime_plugin.TextCommand):
  ...
  def run_script_on_file(self, temp_file_path):
    try:
      ...
      msg = "Command " + '" "'.join(output) + " created invalid output."
      raise Exception(msg)
  ...
  def parse_prettier_option_cli_map(self):
    ...
    for mapping in PRETTIER_OPTION_CLI_MAP:
        ...
        val = settings.get('prettier').get(option_name)
